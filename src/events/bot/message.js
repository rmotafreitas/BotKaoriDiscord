...
module.exports = async (bot, msg) => {
  ...
  afks.findOne(
    ...
    async (err, data) => {
      ...
      if (data) {
        data.delete();
        msg
          .reply("Welcome back! I deleted your afk!")
  ...
  if (msg.mentions.members.first()) {
    ...
    if (afk) {
      msg.channel.send(
        `${afk.nickname} Is Afk: ${afk.afk} | ${moment(afk.time).fromNow()}`
      );
    }
  }
  if (
  ...
  if (msg.channel.name == "kaori-chat" && !msg.author.bot) {
    bot.guilds.cache.forEach(async (guild) => {
      if (guild == msg.guild) return;
      let channel = guild.channels.cache.find((ch) => ch.name === "kaori-chat");
      ...
      let attachment = new Discord.MessageAttachment(img, "chat.png");
